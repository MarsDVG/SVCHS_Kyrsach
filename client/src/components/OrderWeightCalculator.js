...
import { Box, Typography, Paper, TextField, Button, Grid, Card, CardContent, CardMedia, useTheme, useMediaQuery, Fade } from "@mui/material"
import LocalShippingIcon from "@mui/icons-material/LocalShipping"

export default function OrderWeightCalculator({ cars }) {
  ...
  function handleCalculate() {
    ...
    const suitable = cars.filter(car => car.load_capacity >= w)
    if (!suitable.length) {
      setResult({ error: "Нет подходящих автомобилей для данного веса" })
      return
    }
    suitable.sort((a, b) => a.rent_price - b.rent_price)
    setResult({ cars: suitable })
  }
        ...
        <Paper elevation={6} sx={{
          ...
          <Box display="flex" flexDirection={isMobile ? "column" : "row"} gap={2} alignItems={isMobile ? "stretch" : "center"}>
            ...
            <Button
              ...
              sx={{ minWidth: 160, fontWeight: 700, fontSize: isMobile ? 16 : 18, boxShadow: 3, height: isMobile ? 48 : 56, alignSelf: isMobile ? "stretch" : "end" }}
                  ...
                  <Grid container spacing={2}>
                    {result.cars.map(car => (
                      <Grid item xs={12} sm={6} md={4} key={car.id}>
                        <Card sx={{ borderRadius: 3, boxShadow: 6, transition: "0.2s", ":hover": { boxShadow: 12, transform: "scale(1.03)" } }}>
                          {car.img && (
                            <CardMedia
                              component="img"
                              height="120"
